...
const EditLesson = () => {
      ...
      try {
        ...
        if (data.success) {
          ...
          setForm({
            ...
            image: l.image || "📚",
            points: l.points || 50,
            classLevel: l.classLevel || 6,
            videoUrl: l.videoUrl || "",
          });
          setQuiz(l.quiz || []);
        } else {
          setError(data.error?.message || "Failed to load lesson");
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    for (let i = 0; i < quiz.length; i++) {
      const q = quiz[i];
      if (!q.question.trim()) { setError(`Question ${i + 1} is missing text.`); return; }
    ...
    try {
      ...
      if (data.success) {
        ...
        setError(data.error?.message || "Failed to update lesson");
      }
    } catch (err) {
      setError("Failed to connect to server");
    } finally {
          ...
          <div className="max-w-4xl mx-auto">
            ...
            <div className="mb-8 flex items-center justify-between">
              <div>
                ...
                <p className="text-gray-500 mt-1">Update lesson content for Class {form.classLevel}</p>
              </div>
              <button onClick={() => navigate("/admin")} className="text-gray-500 hover:text-gray-700 font-medium">
                ← Back
              </button>
            </div>
              ...
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-5">
                  ...
                  <div>
                    ...
                    <input type="number" name="points" value={form.points} onChange={handleChange} min={10} max={500}
                      className="w-full border border-gray-300 rounded-lg px-4 py-2.5 text-gray-800 focus:outline-none focus:ring-2 focus:ring-primary-500"
                    />
                  </div>
                      ...
                      {EMOJI_OPTIONS.map((emoji) => (
                        <button type="button" key={emoji}
                          ...
                          className={`text-2xl p-2 rounded-lg border-2 transition-all ${
                            form.image === emoji ? "border-primary-500 bg-primary-50 scale-110" : "border-gray-200 hover:border-gray-300"
                          }`}
                        >
                          {emoji}
                        </button>
                ...
                <div className="flex border-b border-gray-100">
                  <button type="button" onClick={() => setActiveTab("content")}
                    className={`flex-1 py-4 text-sm font-semibold transition-colors ${
                      activeTab === "content" ? "text-primary-600 border-b-2 border-primary-600 bg-primary-50" : "text-gray-500 hover:text-gray-700"
                    }`}>
                    📄 Lesson Content
                  </button>
                  <button type="button" onClick={() => setActiveTab("quiz")}
                    className={`flex-1 py-4 text-sm font-semibold transition-colors ${
                      activeTab === "quiz" ? "text-purple-600 border-b-2 border-purple-600 bg-purple-50" : "text-gray-500 hover:text-gray-700"
                    }`}>
                  ...
                  {activeTab === "quiz" && (
                    <div>
                      <div className="flex items-center justify-between mb-5">
                        <p className="text-sm text-gray-500">Add or update quiz questions for this lesson.</p>
                          ...
                          {quiz.map((q, qIdx) => (
                            <div key={qIdx} className="border border-gray-200 rounded-xl p-5">
                              <div className="flex items-center justify-between mb-3">
                                ...
                                <button type="button" onClick={() => removeQuestion(qIdx)} className="text-red-400 hover:text-red-600 text-sm">
                                  Remove
                                </button>
                              </div>
                                  ...
                                  <div key={oIdx} className="flex items-center gap-3">
                                    ...
                                    <input type="text" value={opt}
                                      ...
                                      className={`flex-1 border rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 ${
                                        q.correctAnswer === oIdx ? "border-green-400 bg-green-50 focus:ring-green-300" : "border-gray-300 focus:ring-purple-300"
                                      }`}
                                    />
