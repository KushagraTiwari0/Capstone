  ...
  const fetchTasksAndSubmissions = async () => {
    try {
      ...
      const [tasksRes, subRes] = await Promise.all([
        ...
        fetch(`${API_BASE}/users/me/task-submissions`, { headers })
      ]);
  ...
  const getTaskStatusInfo = (taskId) => {
    ...
    if (submission) {
      if (submission.status === "approved")
        return { label: "✓ Approved", color: "bg-green-100 text-green-800", isDone: true };
      if (submission.status === "rejected")
        return { label: "✗ Rejected", color: "bg-red-100 text-red-800", isDone: false };
      return { label: "⏳ Pending Review", color: "bg-yellow-100 text-yellow-800", isDone: true };
    }
    if (completedTasks?.includes(taskId))
      return { label: "✓ Completed", color: "bg-green-100 text-green-800", isDone: true };
    return { label: "Not Started", color: "bg-gray-100 text-gray-600", isDone: false };
  };
          ...
          <div className="max-w-7xl mx-auto">
            ...
            {!loading && tasks.length === 0 && (
              <div className="flex flex-col items-center justify-center py-16 px-4 bg-white rounded-2xl shadow-sm border border-primary-100 text-center">
                ...
                <Link
                  to="/lessons"
                  className="inline-flex items-center justify-center px-6 py-3 rounded-lg font-semibold bg-primary-600 text-white hover:bg-primary-700 transition-colors"
                >
                  Go to Lessons
                </Link>
              </div>
                ...
                {filteredTasks.length === 0 ? (
                  ...
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 sm:gap-6">
                    {filteredTasks.map((task) => {
                      const { label, color, isDone } = getTaskStatusInfo(task._id);
                      return (
                        <div
                          key={task._id}
                        ...
                        >
                          <div className="p-6">
                            ...
                            >
                              {isDone ? "View Submission" : "Start Task"}
                            </Link>
                          </div>
                        </div>
                      );
