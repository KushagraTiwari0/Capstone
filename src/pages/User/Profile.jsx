...
const getLevelInfo  = (pts) => LEVEL_THRESHOLDS.find(l => pts >= l.min) || LEVEL_THRESHOLDS[3];
const nextLevelInfo = (pts) => { const i = LEVEL_THRESHOLDS.findIndex(l => pts >= l.min); return i > 0 ? LEVEL_THRESHOLDS[i-1] : null; };
...
const Profile = () => {
  ...
  const fetchProfile = async () => {
    try {
      ...
      const prj = await progressRes.json();
      if (pj.success) {
        const u = pj.user;
        const lessonPoints = prj.data?.points ?? u.points ?? 0;
        const gameExp      = prj.data?.exp     ?? u.exp    ?? 0;
        setProfileData({
          ...u,
          points:           lessonPoints + gameExp,
          level:            prj.data?.level            ?? u.level   ?? "Beginner",
          badges:           prj.data?.badges           ?? [],
          completedLessons: prj.data?.completedLessons ?? [],
          completedTasks:   prj.data?.completedTasks   ?? [],
          quizScores:       prj.data?.quizScores       ?? {},
        });
  ...
  const showMsg = (type, msg) => {
    ...
    setSuccessMsg(type === "success" ? msg : "");
  ...
  const handleProfileSave = async (e) => {
    ...
    try {
      const result = await updateUserProfile(user._id || user.id, { name: formData.name.trim(), avatar: formData.avatar });
      if (result.success) {
    ...
    } catch { showMsg("error", "An error occurred. Please try again."); }
  ...
  const handlePasswordSave = async (e) => {
    ...
    try {
      ...
      } else { showMsg("error", data.error?.message || "Failed to change password"); }
    } catch { showMsg("error", "An error occurred. Please try again."); }
      ...
      <div className="flex-1 p-6 lg:p-8">
        <div className="max-w-4xl mx-auto space-y-6 animate-pulse">
          ...
          <div className="bg-white rounded-xl p-8">
            <div className="flex gap-6"><div className="w-20 h-20 bg-gray-200 rounded-full" /><div className="flex-1 space-y-3"><div className="h-6 bg-gray-200 rounded w-1/2" /><div className="h-4 bg-gray-200 rounded w-1/3" /></div></div>
          </div>
        </div>
      </div>
    </div>
  ...
  const avgQuiz      = quizArr.length > 0 ? Math.round(quizArr.reduce((s, q) => s + (q.percentage || 0), 0) / quizArr.length) : null;
    ...
    <div className="flex flex-col lg:flex-row w-full min-h-screen animated-bg">
      ...
      <div className="flex-1 w-full p-4 sm:p-6 lg:p-8">
        <div className="max-w-4xl mx-auto w-full space-y-6">
          ...
          <div>
            <h1 className="text-2xl sm:text-3xl lg:text-4xl font-bold bg-gradient-to-r from-primary-600 to-primary-800 bg-clip-text text-transparent mb-1">My Profile</h1>
            <p className="text-sm text-gray-500">Manage your account and view your progress</p>
          </div>

          {successMsg && <div className="bg-green-50 border border-green-200 text-green-700 px-4 py-3 rounded-lg flex items-center gap-2"><span>✅</span> {successMsg}</div>}
          {errorMsg   && <div className="bg-red-50 border border-red-200 text-red-700 px-4 py-3 rounded-lg flex items-center gap-2"><span>⚠️</span> {errorMsg}</div>}
            ...
            {!isEditing ? (
              <div className="flex flex-col sm:flex-row items-start gap-6">
                ...
                <div className="flex-1">
                  <div className="flex flex-wrap items-center gap-3 mb-1">
                    ...
                    <span className={`text-xs font-bold px-2.5 py-1 rounded-full ${isAdmin?"bg-yellow-100 text-yellow-700":isTeacher?"bg-green-100 text-green-700":"bg-blue-100 text-blue-700"}`}>
                      {isAdmin?"👑 Admin":isTeacher?"👨‍🏫 Teacher":"👨‍🎓 Student"}
                    </span>
                    {/* ✅ Show class level for both students AND teachers */}
                    {!isAdmin && p.classLevel && (
                      <span className="text-xs bg-primary-100 text-primary-700 px-2.5 py-1 rounded-full font-semibold border border-primary-200">
                        📚 Class {p.classLevel}
                      </span>
                    )}
                  </div>
                  ...
                  {isStudent && (
                    <div className={`inline-flex items-center gap-2 mt-1 px-3 py-1.5 rounded-full text-sm font-semibold ${levelInfo.bg} ${levelInfo.color}`}>
              ...
              <form onSubmit={handleProfileSave} className="space-y-5">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {AVATARS.map(av => (
                      <button type="button" key={av}
                        onClick={() => setFormData(f => ({ ...f, avatar: av }))}
                        className={`text-3xl p-2 rounded-lg border-2 transition-all ${formData.avatar===av?"border-primary-500 bg-primary-50 scale-110":"border-gray-200 hover:border-gray-300"}`}>
                        {av}
                      </button>
                    ))}
                  </div>
                ...
                <div className="flex gap-3">
                  <Button type="submit" variant="primary" disabled={savingProfile}>{savingProfile?"Saving...":"Save Changes"}</Button>
                  <Button type="button" variant="outline" onClick={() => { setIsEditing(false); setFormData({ name: p.name, avatar: p.avatar }); }}>Cancel</Button>
                </div>
              </form>
            )}
              ...
              <form onSubmit={handlePasswordSave} className="mt-6 pt-6 border-t border-gray-100 space-y-4">
                ...
                {[
                  ...
                  <div key={key}>
                    ...
                    <input type="password" value={passwordData[key]} placeholder={placeholder}
                      onChange={e => setPasswordData(d => ({ ...d, [key]: e.target.value }))}
                      className="w-full px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-primary-500 outline-none" required />
                  </div>
                ))}
                <Button type="submit" variant="primary" disabled={savingPassword}>{savingPassword?"Saving...":"Update Password"}</Button>
              </form>
            )}
          </div>
          ...
          {isStudent && (
            <>
              {/* ✅ Stats grid — now includes Class Level */}
              <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                {[
                  { icon:"⭐", value:totalPts,           label:"Total Points",  color:"text-yellow-600" },
                  { icon:"📚", value:`Class ${p.classLevel || "–"}`, label:"Class Level",   color:"text-primary-600" },
              ...
              {/* ✅ Student account details row — shows class, email, join date */}
              <div className="bg-white rounded-xl shadow-md p-6">
                <h3 className="text-lg font-bold text-gray-800 mb-4">Account Details</h3>
                <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                  {[
                    { label:"Class Level",  value:`Class ${p.classLevel || "–"}`, icon:"📚" },
                    { label:"Email",        value:p.email,                          icon:"📧" },
                    { label:"Member Since", value:new Date(p.createdAt).toLocaleDateString("en-US",{month:"long",year:"numeric"}), icon:"📅" },
                  ].map(({ label, value, icon }) => (
                    <div key={label} className="bg-gray-50 rounded-lg p-4 flex items-start gap-3">
              ...
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                <div className="w-full bg-gray-100 rounded-full h-3 mb-2">
                  <div className="bg-primary-500 h-3 rounded-full transition-all duration-700" style={{ width:`${levelPct}%` }} />
                </div>
                <p className="text-xs text-gray-500">
                  {nextLevel ? `${ptsToNext} more points to reach ${nextLevel.icon} ${nextLevel.name}` : "🎉 Maximum level reached!"}
                </p>
              </div>
              ...
              <div className="bg-white rounded-xl shadow-md p-6">
                ...
                <div className="space-y-4">
                  {[
                    { label:"Lessons Completed", value:totalLessons,             pct:Math.min(100,(totalLessons/10)*100),          color:"bg-blue-500"   },
                    { label:"Tasks Completed",   value:totalTasks,               pct:Math.min(100,(totalTasks/10)*100),            color:"bg-green-500"  },
                    { label:"Badges Earned",     value:`${earnedBadges.length}`, pct:Math.min(100,(earnedBadges.length/13)*100),  color:"bg-yellow-500" },
                    ...(avgQuiz!==null?[{ label:"Avg Quiz Score", value:`${avgQuiz}%`, pct:avgQuiz, color:"bg-purple-500" }]:[]),
